...
if __name__ == '__main__':
    ...
    if args.dataset == 'h36m':
        ...
        def load_mag_phase(npz_path):
            d = np.load(npz_path)
            mag = d["mag"]

            # phase key 兼容：pha / phase
            if "pha" in d.files:
                phase = d["pha"]
            elif "phase" in d.files:
                phase = d["phase"]
            else:
                raise KeyError(f"No phase key in {npz_path}, keys={d.files}")

            return mag, phase


        def pad_antennas_to_6(x):
            """
            x: [T, A, F]
            A=4 -> 補到 6
            A=6 -> 原樣
            ...
            if A == 6:
                return x
            if A != 4:
                raise ValueError(f"Expected A=4 or 6, got A={A}, shape={x.shape}")
        ...
        class CSI_KPT_Dataset(Dataset):
            def __init__(self, env_roots):
                ...
                for env_path in env_roots:
                    csi_root = os.path.join(env_path, "npy_merged")
            ...
            def _load_and_process_csi(self, csi_path):
                """
                                    統一輸出 shape: [51, 6, 4050]
                - key 兼容：pha / phase
                - 天線數：A=4 pad 到 A=6
                - 只對 mag 做 DWT 去噪（你註解說的）
                """
                mag, phase = load_mag_phase(csi_path)
                mag = mag.astype(np.float32)
                phase = phase.astype(np.float32)

                # ===== 先統一天線數到 6 =====
                mag = pad_antennas_to_6(mag)
                phase = pad_antennas_to_6(phase)

                # ===== 只對 mag 去噪 + normalize =====
                mag = normalize(dwt_denoise(mag))
                phase = normalize(phase)

                # ===== 統一特徵維度到 4050：把 mag+phase 在 subcarrier 維度拼起來 =====
                csi = np.concatenate([mag, phase], axis=2)   # [51,6,4050]
                return csi
            ...
            def __getitem__(self, idx):
                ...
                # ------- 1. 讀「當前」 CSI 並處理 -------
                csi = self._load_and_process_csi(csi_path)  # [51, 6, 4050]
        ...
        #]
        env_roots = [os.path.join(args.root_path, "stand5_train")]
