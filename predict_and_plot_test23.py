...
def collect_env0_test_files():
    return glob.glob("/media/main/HDD/yo/dataset/Env0_ViTPose_huge_test/npy_dwt/*.npz")
...
def main():
    ...
    checkpoint = torch.load('/media/main/HDD/yo/GraphMLP-main/checkpoint/1203_1421_48_1/model_33_3564.pth')
    model.load_state_dict(checkpoint)
    model.eval()

    output_dir = "/media/main/HDD/yo/GraphMLP-main/output_ViTPose_test23"
    ...
    for idx, csi_path in enumerate(tqdm(sampled_files, desc="🔍 推論中")):
        ...
        image_path = csi_path.replace("/npy_dwt/", "/img_nomask/").replace(".npz", ".jpg")
        gt_path = csi_path.replace("/npy_dwt/", "/kpt_npz/")

        csi_npz = np.load(csi_path)
        mag = normalize(csi_npz["mag"].astype(np.float32))
