...
#用Env1訓練
#每一張圖片的每個時間點會和前一張圖片相減得到變化量，以這個變化量作為特徵
import os
import glob
import torch
import random
        ...
        class CSI_KPT_Dataset(Dataset):
            def __init__(self, env_roots):
                ...
                for env_path in env_roots:
                    csi_root = os.path.join(env_path, "npy_dwt_diff")
            ...
            def _load_and_process_csi(self, csi_path):
                """讀取一個 npz 的 CSI，做去噪/正規化，回傳 shape: [51, 6, 4050]"""
                csi_npz = np.load(csi_path)
                mag = csi_npz["mag"].astype(np.float32)
                pha = csi_npz["pha"].astype(np.float32)

                # ===== 這裡目前是「只正規化，不做 DWT / Butter」 =====
                # 若之後要「只對振幅做 DWT 去噪」，可以在這裡改：
                # mag = normalize(dwt_denoise(mag))
                # pha = normalize(pha)
                mag = normalize(mag)
                pha = normalize(pha)

                csi = np.concatenate([mag, pha], axis=2)  # [51, 6, 4050]
                return csi
