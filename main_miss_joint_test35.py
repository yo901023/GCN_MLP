    ...
    if args.dataset == 'h36m':
        ...
        class CSI_KPT_Dataset(Dataset):
            def __init__(self, env_roots, per_env_samples=10000, seed=1, use_dwt_folder="npy",
                ...
                for env_path in env_roots:
                    ...
                    if len(env_pairs) > per_env_samples:
                        ...
                        print(f"✅ {env_name}: sampled {per_env_samples}/{len(csi_files)} (matched {len(env_pairs)})")
                    else:
                        print(f"✅ {env_name}: only {len(env_pairs)} matched (<{per_env_samples}), use all")
            ...
            def __getitem__(self, idx):
                ...
                # 你說「只對振幅做去噪」：就把 mag 做 dwt/normalize，pha 直接 normalize 或不動
