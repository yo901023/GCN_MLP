...
class SubcarrierCrossAttn_Band_Cross(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # -> [B, T, K, A, band] -> [BTK, A, band]
        x_band = x_band.permute(0, 1, 3, 2, 4).contiguous().view(B * T * K, A, self.band_size)

        h = self.embed(x_band)  # [BTK, A, d]

        # 真正 cross-attn：每根天線 i 當 query，只看其他天線當 kv
        outs = []
        for i in range(A):
            q = h[:, i:i+1, :]  # [BTK, 1, d]
            kv = torch.cat([h[:, :i, :], h[:, i+1:, :]], dim=1)  # [BTK, A-1, d]

            yi, _ = self.attn(q, kv, kv)  # [BTK, 1, d]
            yi = self.norm(yi + q)        # residual + norm
            outs.append(yi)
