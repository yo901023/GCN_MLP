...
class PCALayer(nn.Module):
    """
    PCA: z = (x - mean) @ components.T
    等價 Linear: z = x @ W^T + b
      W = components
      b = - mean @ components.T
    """
    def __init__(self, pca_npz_path: str):
        super().__init__()
        p = np.load(pca_npz_path)

        mean = p["mean"].astype(np.float32)           # (24300,)
        comp = p["components"].astype(np.float32)     # (512,24300)

        out_dim, in_dim = comp.shape
        self.proj = nn.Linear(in_dim, out_dim, bias=True)

        W = torch.from_numpy(comp)                    # (512,24300)
        b = - torch.from_numpy(mean) @ W.T            # (512,)
    ...
    def forward(self, x):
        # x: [B,T,in_dim] 或 [B,T,A,F]
        if x.dim() == 4:
            B, T, A, F = x.shape
            x = x.reshape(B, T, A * F)  # [B,T,24300]
        return self.proj(x)             # [B,T,512]

class Model(nn.Module):
    def __init__(self, args, pca_path=None):
        super().__init__()
        ...
        if self.use_pca:
            self.pca = PCALayer(pca_path)   # 24300 -> 512
            self.embedding = nn.Identity()  # ✅ 因為 channel=512，不用再線性層
        else:
            self.embedding = nn.LazyLinear(self.channel)  # 24300 -> 512
    ...
    def forward(self, x):
        ...
        if self.use_pca:
            x = self.pca(x)                 # [B,T,512]
        else:
            x = x.reshape(B, T, A * F)      # [B,T,24300]

        x = self.embedding(x)               # 有 PCA：不變；沒 PCA：變512       
