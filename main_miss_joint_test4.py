...
#針對不同部分給予不同權重、DwT改成沿著時間做去噪
#用graphmlp_attention1(線性層改成attention、51維直接轉成17個關節點)
import os
import glob
import torch
import random
...
def make_joint_weights(rmse, eps=1e-6):
    """
    根據 RMSE 生成關節權重，RMSE 越大權重越小。
    """
    weights = 1.0 / (rmse + eps)
    weights = weights / weights.sum() * len(weights)  # normalize 平均到 1
    return weights
